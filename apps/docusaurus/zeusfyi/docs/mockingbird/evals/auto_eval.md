...
This is the non-case specific state, i.e. not using for filtering or error handling. It's used for informational
purposes
...
This is reserved for now, but acts like an info state for now. We're planning to use it for things like terminating the
workflow
if > 50% of the objects in the result are in an error state, or if the error state is set to terminate the workflow.

If you have any specific use cases for this you like us to include in the feature set please let us know!
support@zeus.fyi
...
When using filtering via EvalState=='filter' the result will be ignored and not used in the final result of group metric
scoring.
